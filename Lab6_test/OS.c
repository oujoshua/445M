...
_OS_Task _tasks[OS_MAX_TASKS]; // static storage for periodic threads
OS_SemaphoreType _modifyTCB;  // binary semaphore for modifying the TCB linked list
_OS_Event _eventLog[_OS_MAX_EVENTS]; // log timestamp data for events
int _eventIndex = 0;  // index for event log
...
void OS_Init(void) {
  ...
  _OS_numThreads = 0;
  // initialize array of periodic tasks to free
